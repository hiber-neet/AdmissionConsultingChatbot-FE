...
// 🔹 ref để luôn giữ convs mới nhất, dùng trong useEffect WebSocket
...
const createConversation = () => {
  ...
  persistConvs(next);
  setActiveId(c.id);
  setMessages([]);          // phiên mới thì rỗng
};

const selectConversation = (id) => {
  setActiveId(id);
  const c = convs.find((x) => x.id === id);
  setMessages(c ? c.messages : []);  // load tin nhắn của phiên đó
};
...
const pushToActive = (msg) => {
  const currentConvs = convsRef.current;    // luôn lấy bản mới nhất
...
useEffect(() => {
  ...
  // Kết nối WS khi vào tab chatbot
   useEffect(() => {
  // chỉ mở WS khi đang ở tab "chatbot" và đã login
  if (tab !== "chatbot" || !user) {
    // rời tab / chưa login -> đóng WS nếu còn mở
  ...
  // đã có kết nối OPEN thì không tạo lại
  if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
    return;
  }

  // dùng chung BASE_URL -> thay http bằng ws
  const wsUrl = (API_BASE_URL || "http://127.0.0.1:8000")
    .replace(/^http/, "ws")
    + "/chat/ws/chat";
  ...
  ws.onopen = () => {
    ...
    // Gửi gói handshake ĐÚNG format BE yêu cầu
    ws.send(
      JSON.stringify({
        user_id: user.id,
        // BE sẽ tự tạo session mới nếu null
  ...
  ws.onmessage = (event) => {
    ...
    switch (ev) {
      case "session_created": {
        // server tạo session mới -> lưu lại để dùng cho các request sau
        if (data.session_id) {
      ...
      case "done": {
        ...
        // hiển thị trên khung chat hiện tại
        setMessages((prev) => [...prev, botMsg]);
        // lưu vào localStorage cho phiên đang chọn
        pushToActive(botMsg);

        // reset trạng thái stream
        partialRef.current = "";
        setPartialResponse("");
        setIsLoading(false);
        break;
  ...
  // Đẩy tin nhắn lỗi như 1 message của bot
...
}, [tab, user]); // giữ dependency như cũ
...
const handleSend = (e) => {
  ...
  // đẩy tin user lên UI + lưu vào phiên hiện tại (local)
  const userMsg = { sender: "user", text };
  setMessages((prev) => [...prev, userMsg]);
  pushToActive(userMsg);

  // Gửi JSON cho BE: BE chỉ cần field "message", user_id, session_id
  wsRef.current.send(
    JSON.stringify({
      message: text,
      user_id: user.id,
      // luôn dùng session hiện tại mà BE đã tạo / gửi về
      session_id: chatSessionIdRef.current ?? null,
    })
  );

  // reset input + trạng thái stream bot
  setInput("");
  setPartialResponse("");
  partialRef.current = "";
  setIsLoading(true);
