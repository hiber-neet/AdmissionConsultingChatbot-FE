...
// Role mapping from role_id to role name (matching backend exactly)
const roleMapping = {
  1: 'Admin',
  2: 'Consultant', 
  3: 'Content Manager',
  4: 'Admission Official',
  5: 'Student',
  6: 'Parent',
...
const PERMISSION_NAME_TO_ID = {
  ...
  'Consultant': 2,
  'Content Manager': 3,
  'Admission Official': 4,
  // Handle legacy lowercase versions for compatibility during migration
  'admin': 1,
  'consultant': 2,
  'content_manager': 3,
  'admission_officer': 4,
  ...
  if (apiUser.permissions && Array.isArray(apiUser.permissions)) {
    ...
    permissions = apiUser.permissions.map(perm => {
      if (typeof perm === 'object' && perm.permission_id) {
        ...
        // If it's already a permission name, use the exact backend format
        return perm; // Keep original case and spaces for backend compatibility
      } else if (typeof perm === 'number') {
        return PERMISSION_ID_TO_NAME[perm] || 'Student';
      }
      return roleName; // Fallback to role name with proper formatting
    });
    
    // Remove duplicates and ensure at least the role is included
    permissions = Array.from(new Set([...permissions, roleName]));
  } else {
...
const initialUsers = [
  ...
  },
  {
    ...
    permissions: ['Content Manager', 'Consultant'],
    status: 'active',
  ...
  },
  {
    ...
    role: 'Admission Official',
    permissions: ['Admission Official'],
    status: 'active',
  ...
  },
  {
    ...
    role: 'Admission Official',
    permissions: ['Admission Official', 'Consultant'],
    status: 'active',
      ...
      if (!response.ok) {
        ...
        try {
          ...
          if (response.status === 403 && parsedError.detail === "Admin permission required") {
            ...
            console.error('   1. User role in backend is not Admin');
    ...
    try {
      ...
      if (editingUser) {
        ...
        if (permissionsChanged) {
          ...
          // or having Admin/Content Manager permissions along with consultant role, etc.
          const consultantIsLeader = formData.role === 'Consultant' && 
            (newPermissions.includes('Admin') || newPermissions.includes('Content Manager') || newPermissions.length > 1);
          const contentManagerIsLeader = formData.role === 'Content Manager' && 
        ...
        const consultantIsLeader = formData.role === 'Consultant' && 
          (formData.permissions.includes('Admin') || formData.permissions.includes('Content Manager') || formData.permissions.length > 1);
        const contentManagerIsLeader = formData.role === 'Content Manager' && 
